...
深度来自夜色的渐变层、细框、柔光与少量暗阴影。背景和机关在最下方；HTML 界面、提示、遮罩依次叠放；青色光点始终位于可见界面之上。弹层以深色遮罩隔开场景，维持清楚的操作层次。
...
细框直角、最小高度 44 px；主按钮青灰填色，次按钮深夜填色。桌面悬停和按下由模拟光点命中产生，菜单光点不提速。按下有 1 px 下沉；禁用态减淡并保留语义。键盘焦点为外移 4 px 的 2 px 暖金轮廓，Tab、Enter 与 Esc 路径保留。

### Inputs / Fields

深色直角输入框，细线边界、暖金插入符，与按钮共用焦点轮廓。手机使用原生触控与输入法；桌面点击定位跟随虚拟光点，键盘选择和中文输入法仍为原生行为。服务未配置时显示真实不可用状态并禁用提交区。
