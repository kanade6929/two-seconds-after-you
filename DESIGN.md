...
触控模式由粗指针媒体条件或首次触摸启用，不只依赖窗口宽度。竖屏游戏把标题、谜题、状态、拇指控制依次分区；底部光域高 126 px，为一整块语义化按钮，合并移动与确认，安全区由 `env(safe-area-inset-*)` 留白。滑动超过 8 CSS px 才开始移动并永久取消本次轻点；原地按住 0.22 秒记录持续回应，移动立即松开。滑动为青色、按住为暖金反馈，暂停、换阵、取消和失去触点捕获都清理手势；不把长按松手当成确认。牌章重排为两列，标题与社区的返回按钮禁止压缩换行。
