// Comparative desktop Chrome / mobile viewport benchmark, not physical-phone FPS.
...
async function run(label,revision){
 ...
 if(revision)for(const file of ['render.js','app.js']){const source=execFileSync('git',['-c','safe.directory='+root.replaceAll('\\','/'),'show',revision+':'+file],{cwd:root,encoding:'utf8'});await p.route('**/'+file,r=>r.fulfill({body:source,contentType:'text/javascript'}));}
 ...
 await p.screenshot({path:path.join(dir,'performance-'+label+'.png')});await browser.close();console.log(JSON.stringify({label,...result}));return result;
}
(async()=>{const baseline=await run('before','9ebedf9'),current=await run('after');fs.writeFileSync(path.join(dir,'performance-comparison.json'),JSON.stringify({baseline,current},null,2));})().catch(e=>{console.error(e);process.exitCode=1;});
