...
test('no early door click, no afterglow eligibility, no permanent power',()=>{const d=driver(0),g=d.g;d.move(g.level.exit,2);g.update(0,g.point,true);assert.equal(g.won,false);d.move(g.level.seal,3);d.move(g.level.exit,1.5);assert.equal(g.open,true);d.move(g.level.exit,1);assert.equal(g.open,false);g.update(0,g.point,true);assert.equal(g.won,false);assert.equal(g.exitHold,0);d.exit();assert.equal(g.won,true);});
