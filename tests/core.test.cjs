...
test('no early door click, no afterglow eligibility, no permanent power',()=>{const d=driver(0),g=d.g;d.move(g.level.exit,2);d.click();assert.equal(g.won,false);d.move(g.level.seal,3);d.move(g.level.exit,1.5);assert.equal(g.open,true);d.move(g.level.exit,1);assert.equal(g.open,false);d.click();assert.equal(g.won,false);assert.equal(g.hold,0);d.move(g.level.seal,3);d.until(()=>g.open,g.level.exit);d.click();assert.equal(g.won,true);});
test('checkpoint retains stable phase but never temporary power or partial echo',()=>{const g=new Game(2,{phase:1});assert.equal(g.state.phase,1);assert.equal(g.echo,null);assert.equal(g.open,false);assert.equal(g.timeline.samples.length,1);const d=driver(0);d.move(d.g.level.seal,3);const fresh=new Game(0,d.g.checkpoint());assert.equal(fresh.state.phase,1);assert.equal(fresh.open,false);assert.deepEqual(fresh.contacts,{});});
