...
test('early clicks are never queued and old power cannot open a door',()=>{const d=driver(0);d.tap(d.g.level.exit);d.move(d.g.level.seal,3.2);d.move(d.g.level.exit,1.3);assert.equal(d.g.open,true);assert.equal(d.g.won,false);d.move(d.g.level.exit,3);d.click();assert.equal(d.g.won,false);});
test('checkpoint retains choices but never restores a powered door or partial replay',()=>{const d=driver(1),R=require('../rules.js');d.tap(R.magicLayout().mirrors[0]);d.move(R.magicLayout().source,3);const fresh=new Game(1,d.g.checkpoint());assert.deepEqual(fresh.state.orientations,[1,0]);assert.equal(fresh.echo,null);assert.equal(fresh.memory,null);assert.equal(fresh.open,false);assert.equal(fresh.timeline.samples.length,1);});
