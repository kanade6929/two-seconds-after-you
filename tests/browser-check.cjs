...
async function run(channel){
 ...
 async function press(on){if(mobile){const b=await page.locator('#touchPad').boundingBox();await cdp.send('Input.dispatchTouchEvent',{type:on?'touchStart':'touchEnd',touchPoints:on?[{x:b.x+b.width/2,y:b.y+b.height/2}]:[]});}else if(on)await page.mouse.down();else await page.mouse.up();}
 ...
 for(let index=0;index<8;index++){
  ...
  if(index===2)for(let phase=0;phase<3;phase++){await advance(.5);const [past,now]=R.loversPairs[phase];await move(past,1.6);await press(true);await advance(1.5);await press(false);await move(now,.95);await press(true);await advance(mobile?1.05:.8);await press(false);assert.ok((await state()).phase>phase||(await state()).won,'paired press '+phase);}
  ...
  if(index===5)for(let phase=0;phase<3;phase++){await advance(.5);await move(R.moonWell,3);await until(g=>g.open,R.moonOptions[R.moonMaps[phase].indexOf(R.moonTargets[phase])]);await click();assert.ok((await state()).phase>phase||(await state()).won);}
  if(index===6)for(let phase=0;phase<3;phase++){await advance(.5);const m=R.sunLayout(phase);await move({x:780,y:490},2);await move({x:400,y:490},2);await move(m.pads[0],3);const y=phase===1?408:322;await move({x:565,y},.45);await move({x:635,y},.35);await move(m.pads[1],.5);if(phase===2){await press(true);await advance(1);await press(false);}else await until(g=>g.phase>phase,m.pads[1],3);}
