(function(root){
  ...
  const LEVELS=[
    ...
    {id:'moon',title:'月亮',roman:'XVIII',phases:3,theme:'暂时看不清，也可以慢慢辨认。',task:'先记住上方图腾；月井启动后提示隐去，每轮只选一枚真纹。',done:'有些微光隐去了，你仍记得它的模样。',hint:['月井未启动时，上方显示本轮要记住的图腾；启动后提示隐藏，只留下三个候选纹样。','先看清上方的一枚图腾，在月井留下停留，再去点击与记忆相同的真纹。忘记时等影子离井，目标会重新出现。每轮只选一次，选对三轮即完成。']},
  ...
  const moonWell=point(560,435),moonOptions=[point(390,280),point(560,260),point(730,280)],moonMaps=[['太阳','月亮','星星'],['星星','太阳','月亮'],['月亮','星星','太阳']],moonTargets=['星星','太阳','月亮'];
  ...
  const modules={
    ...
    moon(g,dt,click){const s=g.state,past=occupy(g,'well',g.echo,moonWell),revealed=occupancy(g,'well',past,dt);let at=-1;moonOptions.forEach((v,i)=>{if(occupy(g,'choice'+i,g.point,v))at=i;});if(g.effect.at!==at)g.hold=0;g.effect={revealed,at};
      // Readiness must not reveal correctness: every visible choice settles alike.
      charge(g,revealed&&at>=0,dt,.18);
      if(click&&g.open){if(moonMaps[s.phase][at]===moonTargets[s.phase])advance(g,moonOptions[at]);else{s.error=1.2;g.hold=0;g.open=false;}}
    },
  ...
  function view(g){
    ...
    if(l.id==='moon'){const revealed=!!g.effect.revealed;o.moonMemory={concealed:revealed,target:revealed?null:moonTargets[s.phase]};o.nodes=[n('well',moonWell,'月井 · 昔','月亮','echo',held(g,'well'),{amount:(g.timers.well||0)/.3}),...moonOptions.map((p,i)=>n('choice'+i,p,revealed?'选择真纹':'倒影',revealed?moonMaps[s.phase][i]:'月亮','any',held(g,'choice'+i),{choice:true,exit:g.open&&g.effect.at===i,amount:g.effect.at===i?g.hold/.18:0}))];o.message=s.error?'这枚与记忆不同。本轮还在，可再辨认；忘记时等月井熄灭，重看目标。':revealed?'目标已隐去。凭记忆选择一枚真纹，点击确认。':'记住上方这一枚图腾，再让影子守住月井。每轮只需选一枚。';if(revealed)o.actionPoint=moonOptions[g.effect.at];}
    ...
    const clickLevels=['fool','magician','star','moon','world'];o.clickable=clickLevels.includes(l.id)&&g.open&&!g.won;if(o.clickable)o.actionLabel=l.id==='world'?'点击联转':l.id==='moon'?'确认选择':s.phase===l.phases-1?'点击完成':'点击封存';return o;
  }
