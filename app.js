(function(){
  ...
  let likeInfo=null;
  function likeView(info){likeInfo={liked:!!info.liked,count:Number(info.count)||0};$('likeLabel').textContent=`${likeInfo.liked?'已赞':'点赞'} · ${likeInfo.count}`;$('likeButton').setAttribute('aria-pressed',String(likeInfo.liked));}
  let liking=false;$('likeButton').onclick=async()=>{if(!community.configured){$('commentsButton').click();return;}if(liking||!likeInfo)return;const previous={...likeInfo},optimistic={liked:!previous.liked,count:Math.max(0,previous.count+(previous.liked?-1:1))};liking=true;likeView(optimistic);$('likeButton').classList.add('is-committing');$('likeButton').setAttribute('aria-busy','true');try{likeView(await community.toggleLike(previous.liked));}catch{likeView(previous);$('likeLabel').textContent='点赞未送达 · 重试';}finally{liking=false;$('likeButton').classList.remove('is-committing');$('likeButton').removeAttribute('aria-busy');}};
